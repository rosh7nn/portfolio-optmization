...
from mv_backtest import run_mv_backtest, export_mv_metrics
...
def load_log_returns(filename: str = 'data/log_returns.csv') -> pd.DataFrame:
    try:
        ...
        if set(log_returns.columns) != set(NIFTY50_TICKERS):
            print(f"Warning: Loaded log returns columns do not match NIFTY50_TICKERS")
            return None
...
def forecast_arima_garch(returns: pd.Series, n_paths: int = BOOTSTRAP_PATHS, 
    ...
    try:
        ...
        print(f"Error in ARIMA-GARCH forecasting for {returns.name}: {e}")
...
def forecast_xgboost(returns: pd.Series, lags: int = 11, 
    ...
    try:
        ...
        print(f"Error in XGBoost forecasting for {returns.name}: {e}")
        return np.nan, np.nan
...
def optimize_portfolio(mu: np.ndarray, sigma: np.ndarray, log_returns: pd.DataFrame, 
                      rebalance_dates: pd.DatetimeIndex, objective: str = 'gmv', 
    ...
    n_assets = len(mu)
    # Run Mean-Variance backtest only for the specified objective
    mv_results = run_mv_backtest(log_returns, rebalance_dates, objective)
    export_mv_metrics(mv_results, f'MV {objective.upper()}')
...
def run_backtest(log_returns: pd.DataFrame, rebalance_dates: pd.DatetimeIndex, 
    ...
    for i, date in enumerate(log_returns.index[ESTIMATION_WINDOW:], start=ESTIMATION_WINDOW):
        ...
        if date in rebalance_dates:
            ...
            new_weights = optimize_portfolio(mu, sigma, log_returns, rebalance_dates, objective=objective)
...
def plot_backtest_results(results: Dict[str, pd.Series], title: str = 'Portfolio Performance', 
    ...
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig(f'plots/{title.lower().replace(" ", "_")}_portfolio.png', dpi=300, bbox_inches='tight')
    plt.close()
    if 'daily_returns' in results:
        ...
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.tight_layout()
        plt.savefig(f'plots/{title.lower().replace(" ", "_")}_drawdown.png', dpi=300, bbox_inches='tight')
        plt.close()
...
def main(use_cached_data: bool = True):
    ...
    benchmark_returns = benchmark_returns[first_rebalance_date:]
